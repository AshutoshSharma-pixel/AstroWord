...
const PITRA_TAGLINES = [
  "Examining Sun conjunct node boundaries...",
  "Analyzing 9th house cusp and aspects...",
  "Evaluating 9th lord strength and placement...",
  "Determining ancestral karma severity...",
  "Your Pitra Dosha reading is ready..."
];

const SUGGESTED_QUESTIONS = [
  "How does my Pitra Dosha severity affect my career path?",
  "What is the difference between Kaal Sarp and Pitra Dosha?",
  "Which ancestor is represented by my Sun-node conjunction?",
  "What are the best remedies to perform during Pitru Paksha?",
  "How does my current Mahadasha lord interact with my 9th house?"
];
...
export default function PitraDoshaPage() {
  ...
  useEffect(() => {
    if (isLoading && chartData && !result) {
      const interval = setInterval(() => {
        setTaglineIndex(prev => (prev < PITRA_TAGLINES.length - 1 ? prev + 1 : prev));
      }, 2000);
      return () => clearInterval(interval);
    }
  }, [isLoading, chartData, result]);

  const fetchPitraDosha = async (chart: any) => {
    setIsLoading(true);
    setError(null);
    try {
      const idToken = user ? await user.getIdToken() : '';
      const res = await fetch(`${API_URL}/api/pitra-dosha`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        setError('Could not calculate your Pitra Dosha details. Please try again.');
        setIsLoading(false);
        return;
      }
    ...
    return (
      <div className="min-h-[100dvh] bg-bg flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          ...
          <p className="text-white font-serif text-lg">The karmic analysis failed</p>
          <p className="text-muted text-sm">{error}</p>
          <button
            onClick={() => { setError(null); if (chartData) fetchPitraDosha(chartData); }}
  ...
  return (
    <div className="min-h-[100dvh] bg-bg text-text py-12">
      <TopToolsStrip currentTool="pitra-dosha" />
      <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6 animate-in slide-in-from-bottom-8 duration-700">
        <div className="bg-surface2 border border-gold/30 rounded-2xl p-6 sm:p-8 text-center space-y-3 sm:space-y-4 relative overflow-hidden">
          ...
          <p className="text-muted text-xs uppercase tracking-widest font-mono">
            Pitra Dosha Check Result
          </p>
          <h1 className="font-serif text-4xl sm:text-5xl tracking-wide py-2 text-gold">
            {result.has_pitra_dosha ? 'Pitra Dosha Active' : 'No Pitra Dosha'}
          </h1>
          <p className="text-white text-sm tracking-wide">
            Severity: <span className={result.has_pitra_dosha ? 'text-red font-semibold' : 'text-emerald-400 font-semibold'}>{result.severity}</span>
          </p>
          {result.has_pitra_dosha && result.reasons && result.reasons.length > 0 && (
            <div className="flex flex-wrap gap-2 justify-center pt-2 max-w-md mx-auto">
              {result.reasons.map((reason: string, i: number) => (
                <span key={i} className="text-xs border px-3 py-1.5 rounded-full font-mono text-red border-red/20 bg-red/10">
                  {reason}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-2 justify-center pt-2">
            <span className="text-xs border px-4 py-1.5 rounded-full font-mono text-white/80 border-border bg-surface">
              Sun in House: {result.sun_house}
            </span>
            <span className="text-xs border px-4 py-1.5 rounded-full font-mono text-white/80 border-border bg-surface">
              Rahu in House: {result.rahu_house}
            </span>
            <span className="text-xs border px-4 py-1.5 rounded-full font-mono text-white/80 border-border bg-surface">
              Ketu in House: {result.ketu_house}
            </span>
          </div>
        </div>

        <ShareCard
          question="Am I affected by Pitra Dosha?"
          answer={result.has_pitra_dosha ? `Yes (${result.severity} Severity)` : 'No Pitra Dosha'}
          subtext={result.has_pitra_dosha ? `Active Factors: ${result.reasons.join(', ')}` : 'No significant indicators in chart'}
          keywords={['Pitra Dosha', result.severity, 'Ancestral Karma', 'Astrology']}
        />
        ...
        <div className="mt-6 bg-surface2 border border-gold/20 rounded-2xl p-5 text-center space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-2">
            <button
              onClick={() => {
                sessionStorage.setItem('pending_question', `What are the detailed ancestral karma remedies I should do for my ${result.severity} severity Pitra Dosha?`);
            ...
            <button
              onClick={() => {
                sessionStorage.setItem('pending_question', `How can I perform basic ancestor tarpan or feeding rituals at home?`);
                window.location.href = '/';
              }}
